...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (sleepTruth) drawSleepIcon(canvas, height, width);
		}
		
		private float sleep_start, sleep_end;
		private boolean sleepTruth = false;
		public void setSleepTruth(float start, float end){
			this.sleepTruth = true;
			this.sleep_start = start;
			this.sleep_end = end;
		}
		
		public void removeSleepTruth(){
			this.sleepTruth = false;
		}
		...
		public boolean onTouchEvent(MotionEvent event) {
			//TODO
//			Log.d("GraphView", title);
//			if (syncGraph != null){
//				syncGraph.onTouchEvent(event);
////				Log.d("GraphView", syncGraph.getTitle());
//			}
//			else 
//				Log.d("GraphView", "null");
	...
	private class VerLabelsView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (getGraphViewStyle().getVerticalLabelsWidth()==0 && getLayoutParams().width != verLabelTextWidth+GraphViewConfig.BORDER) {
				setLayoutParams(new LayoutParams(
						(int) (verLabelTextWidth+GraphViewConfig.BORDER), android.view.ViewGroup.LayoutParams.FILL_PARENT));
			} else if (getGraphViewStyle().getVerticalLabelsWidth()!=0 && getGraphViewStyle().getVerticalLabelsWidth() != getLayoutParams().width) {
				setLayoutParams(new LayoutParams(
						getGraphViewStyle().getVerticalLabelsWidth(), android.view.ViewGroup.LayoutParams.FILL_PARENT));
			}
	...
	private GraphView syncGraph;
	public void setSyncGraph(GraphView gVi){
		this.syncGraph = gVi;
	}
	...
	public GraphView(Context context, AttributeSet attrs) {
		...
		int width = attrs.getAttributeIntValue("android", "layout_width", android.view.ViewGroup.LayoutParams.MATCH_PARENT);
		int height = attrs.getAttributeIntValue("android", "layout_height", android.view.ViewGroup.LayoutParams.MATCH_PARENT);
	...
	public GraphView(Context context, String title) {
		...
		addView(graphViewContentView, new LayoutParams(android.view.ViewGroup.LayoutParams.FILL_PARENT, android.view.ViewGroup.LayoutParams.FILL_PARENT, 1));
	}
	...
	protected void drawSleepIcon(Canvas canvas, float height, float width) {
		// TODO
//		canvas.dr
	}
...
//			labels[i] = formatLabel(min + ((max-min)*i/numLabels), true);
			labels[i] = formatLabel(getValueAt(i, numLabels, true), true);
		}
		return labels;
	}

	private double getValueAt(int index, int total, boolean isValueX){
		double highest = 0;
		if (graphSeries.size() > 0) {
			...
			if (values.length == 0) {
				...
				int position = Math.round((values.length-1) * index / total);
				highest = values[position].getX();
			}
			for (int i=1; i<graphSeries.size(); i++) {
				values = graphSeries.get(i).values;
				int position = Math.round((values.length-1) * index / total);
				highest = Math.max(highest, values[position].getX());
